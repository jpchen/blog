...
The approach is to treat intuitive physics as a pattern recogntion problem.
 ...
 Josh also talked about probabilistic programs and its applications in inverse graphics.  *Inverse graphics* is the problem of taking a 2D and derendering it into its components (usually in 3D).
...
Noah Goodman gave talk at the "Learn how to code a paper with state of the art frameworks" workshop about our universal probabilistic programming language, [Pyro](http://pyro.ai).  You can find the jupyter notebook corresponding to the talk [here](https://notebooks.azure.com/mltrain/libraries/MLTrain-NIPS-2017/tree/pyro).

## Papers

Below is a small subset of papers I found interesting.  I want to preface that the below is *heavily* subjected to selection bias, and I am fully aware that I may be omitting outstanding papers that deserve mention.  I am also aware that this is machine learning after all and as such, these papers have been out for a while and may seem rather old to some readers.  Nonetheless, they were published at NIPS so here they are:

**Capsule Networks** -- Sabour et al [\[1\]](#1) released a paper on Capsule Networks.  This was an older idea by Hinton in which a group of neurons act as a "capsule" whose activity vector represents some property.  In a layered architecture, lower level capsules with strong signals can activate higher capsules ("routing by agreement"). They demonstrated SOTA results in MNIST and noted that multi-layer capsule networks perform better than standard CNNs on Multi-MNIST.

**REBAR** -- In the world of probabilistic machine learning, variance reduction has always been the name of the game.  One of the main drawabacks of probabilistic methods in high dimensions is high variance, and as such, techniques to reduce variance have made big strides in this area. Tucker et al introduced REBAR [\[2\]](#2), a combination of the REINFORCE estimator and Concrete relaxation of discrete variables to produce low variance, unbiased estimates.  This crucially leverages the reparameterization trick, which allows low variance estimates of models with continuous variables.  There has since been other research on top of this, notably *RELAX* [\[3\]](#3), a more general form of the REBAR estimator, which is under review for ICLR.

**Trust VI** -- Regier et al introduced TrustVI [\[4\]](#4), a black box variational inference method that uses trust region optimization and has magnitudes better performance than ADVI [\[5\]](#5).  The algorithm roughly works as follows:
...
## Events
### Uber @ NIPS
Uber hosted a party (joint between AI Labs and ATG) on the second night. It was completely full, and I had great conversations with some really interesting folks!
