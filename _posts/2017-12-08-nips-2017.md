...
Because of the sheer number of submissions and attendees, most of the events and proceedings were run massively in parallel, which forced prioritizing and compromise for every talk throughout the day.  Unfortunate, yet unavoidable.  Workshops provided a great venue to engage with presenters and deep dive on a few specific posters without feeling the pressure of burdening everyone else with my questions.
...
**REBAR** -- In the world of probabilistic machine learning, variance reduction has always been the name of the game.  One of the main drawabacks of probabilistic methods in high dimensions is high variance, and as such, techniques to reduce variance have made big strides in this area. Tucker et al [\[2\]](#2) introduced REBAR, a combination of the REINFORCE estimator and Concrete relaxation of discrete variables to produce low variance, unbiased estimates.  This crucially leverages the reparameterization trick, which allows low variance estimates of models with continuous variables.  There has since been other research on top of this, notably *RELAX* [\[3\]](#3), a more general form of the REBAR estimator, which is under review for ICLR.
...
**Alpha Zero** -- David Silver gave a talk on Alpha Zero [\[8\]](8), the "successor" to [AlphaGo Zero](https://deepmind.com/blog/alphago-zero-learning-scratch/).  The paper is well worth a read (especially if you are a chess enthusiast) if you are familiar with DeepMind's previous achievements with [AlphaGo](https://deepmind.com/research/alphago/).  The AlphaGo team was able to use the same RL algorithm with the same initialization and surpass previous SOTA in three different games.  There has been some pushback in the community regarding how "general" this algorithm is and the metrics in which it was evaluated, but it is nonetheless a very impressive RL result.
