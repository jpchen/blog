...
**Capsule Networks** -- Sabour et al [\[1\]](#1) released a paper on Capsule Networks.  This was an older idea by Hinton in which a group of neurons act as a "capsule" and if a threshold of neurons fired, that capsule would become "active". They demonstrated SOTA results in MNIST.
TODO

**REBAR** -- In the world of probabilistic machine learning, variance reduction has always been the name of the game.  One of the main drawabacks of probabilistic methods in high dimensions is high variance, and as such, techniques to reduce variance have made big strides in this area. REBAR [\[2\]](#2) is the result of  REINFORCE estimator and Concrete, TODO
There has since been another paper building off of this, *RELAX* [\[3\]](3), which is under review for ICLR.

**Trust VI** -- Regier et al introduced TrustVI [\[4\]](4), a black box variational inference method that uses trust region optimization and has better performance than ADVI [\[5\]](5).

**Hierarchical Modeling with RNNs** Hafner et al [\[6\]](6) used an RNN that was able to learning information flow and connections in a hierarchical domain.

**Inverse Graphics** Wu, Wang, et al introduced MarrNet [\[7\]](7), a neural net that is able to reconstruct 3D objects from 2.5D images.

**Neural Program Induction** --
Surabh's

**Alpha Zero** -- David Silver gave a talk on Alpha Zero, the "successor" to AlphaGo Zero.  The paper is well worth a read (especially if you are a chess enthusiast).  The AlphaGo team was able to use the same RL algorithm with the same initialization and surpass previous SOTA in three different games.  There has been some pushback in the community regarding how "general" this algorithm is and the metrics in which it was evaluated, but it is nonetheless a very impressive RL result.
...
## References
<a name="1"></a> [1] Sabour et al. *Dynamic Routing between Capsules.*

<a name="2"></a> [2] Sabour et al. *Dynamic Routing between Capsules.*

<a name="3"></a> [3] Sabour et al. *Dynamic Routing between Capsules.*
